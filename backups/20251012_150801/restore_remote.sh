...
gunzip -f portfolio_backup_20251012_150801.sql.gz

echo "Detecting tables from backup file..."
TABLES=$(grep -oP "(?<=INSERT INTO )[^ ]+" portfolio_backup_20251012_150801.sql | sort -u | tr '\n' ',' | sed 's/,$//')
...
if ! head -1 portfolio_backup_20251012_150801.sql | grep -q "encoding UTF8"; then
  sed -i '1i \encoding UTF8' portfolio_backup_20251012_150801.sql
fi

# Restore with proper UTF-8 handling
cat portfolio_backup_20251012_150801.sql | docker exec -i portfolio-postgres psql -U postgres -d portfolio_db --set client_encoding=UTF8 --set ON_ERROR_STOP=off
