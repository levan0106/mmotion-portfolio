...
async function cleanupFundData(portfolioId?: string): Promise<void> {
  console.log('≡ƒº╣ Starting Fund Data Cleanup...\n');
  ...
  try {
    ...
    console.log('Γ£à Connected to database');
    ...
    if (portfolioId) {
      ...
      if (result.rows.length === 0) {
        console.error(`Γ¥î Portfolio ${portfolioId} not found`);
        return;
      }
    ...
    if (portfolios.length === 0) {
      console.log('Γä╣∩╕Å  No funds found to clean up');
      return;
    }
    ...
    for (const portfolio of portfolios) {
      console.log(`≡ƒôè Processing portfolio: ${portfolio.name} (${portfolio.portfolio_id})`);
      
      console.log(`   ≡ƒôê Before cleanup:`);
      ...
      console.log(`   ≡ƒÄë Portfolio ${portfolio.name} cleaned successfully!\n`);
    }

    // Print summary
    console.log('≡ƒôï CLEANUP SUMMARY');
    console.log('==================');
    ...
    results.forEach((result, index) => {
      ...
      console.log(`   - Portfolio Reset: ${result.portfolioReset ? 'Γ£à' : 'Γ¥î'}`);
    });
    ...
    console.error('Γ¥î Error during cleanup:', error);
    throw error;
  } finally {
    await client.end();
  }
...
async function main() {
  ...
  try {
    ...
    console.log('\nΓ£à Fund data cleanup completed successfully!');
    process.exit(0);
  } catch (error) {
    console.error('\nΓ¥î Fund data cleanup failed:', error);
    process.exit(1);
  }
}
