...
chmod +x solution/backend/scripts/setup-local.sh
./solution/backend/scripts/setup-local.sh

# Alternative: Manual setup with Docker
cd solution/backend && npm install && cd ../frontend && npm install && cd ../..
...
- [x] Copy `solution/backend/env.example` to `solution/backend/.env` and set database/redis URLs
...
- [x] Run migrations `cd solution/backend && npm run typeorm:migration:run`
