...
Write-Host "  1.1. Extract: gunzip $LocalBackupDir\$BackupName.sql.gz"
Write-Host "  1.2. Restore: docker exec -i $ContainerName psql -U $DbUser -d $DbName -f $LocalBackupDir\$BackupName.sql"
Write-Host "  1.3. Or use restore script: bash $LocalBackupDir\restore_full.sh"
Write-Host "  2. Or use restore script: .\scripts\restore-database-from-backup.ps1 -BackupFile $LocalBackupDir\$BackupName.sql.gz"
